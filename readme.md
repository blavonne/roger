...
```shell script
#from guest
sudo tail /var/log/fail2ban.log | grep Ban
#result
...
фильтр filtername.conf, после чего выполните `sudo fail2ban-regex /var/log/apache2/access.log /etc/fail2ban/filter.d/filtername.conf`.
...
Настало время атаковать сервер с помощью Slowloris.
```shell script
#from guest
sudo apt-get install tcpdump            #для визуализации Slowloris
sudo tcpdump 192.168.1.65               #лог запросов от хоста (подставить свой) в реальном времени
```